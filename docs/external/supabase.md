...
* Supabase?? ?? ??? ??? Next.js App Router? `app/api/*` ????? ???? **Hono ??????? ??? ??**??.
* Row Level Security(RLS)? ???? ??? ????, **?? ????? ????Rate Limit??? ??? Hono ???? ??**??.
* Supabase **Service Role Key? Next.js ?? ?? ???? ??**??, React ?????? ???? ???? ?? ???? ???.
* React ?????? `/api/*` ??? Hono ?????? ????, Hono? Supabase REST ?? SDK ??? ?? ??? ? ??? ??? ????.
* Hono? ?? ??? ?? ?? ???? ??/???? ? Supabase? ???, ???? ?? ?? ??? ??? ???? DB? ??? ??? ??.
...
### 2-5. Hono ???? ? SDK ?? ?? (Next.js App Router)

?? ????? Next.js `app/api/*` ??? ??? Hono ??? Supabase SDK? ?? ????. Hono? ??? ??? ?, ??? ? ?? Supabase? ???? ??? ??? ????.
...
* **Hono ????**
  ...
  * ??: Next.js `app/api/*` ??? + `@hono/vercel` + Vercel Serverless Functions
